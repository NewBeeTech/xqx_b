<template>
	<div class="main">
		...
		<div class="grousList">
			...
			<div class="openDetail" @click="navGo('/pages/create-makeGroups/main?id='+id+'&onlyView=true')">
				<p>查看拼团活动详情</p>
				<img src="/static/imgs/right.png" />
			</div>
		</div>
		<div class="market">
			注：客户参加活动拼团成功后，支付的款项会在您预设的返金比例扣除返金金额后才能进入您的小确幸账户中～
		</div>
		<div class="groupsDetailMsg">
			<div class="detailTab" v-if="!close">
				<p @click="changeShow2" :class="{active:isShow}">已拼团</p>
				<p @click="changeShow1" :class="{active:!isShow}">拼团中</p>
			</div>
			<!--已拼团-->
		...
		<div v-if="!close" class="makeGroups" @click="makeGroups">下架拼团</div>
	</div>
